...
public interface ProgrametoQueryLogMapper {
	...
	BasicColumn[] selectList = BasicColumn.columnList(id, time, cost, openid, unionid, query, sectionKey, isDetail,
			hasResult, result);
	...
	default int insert(ProgrametoQueryLog record) {
		return MyBatis3Utils.insert(this::insert, record, programetoQueryLog,
				c -> c.map(time).toProperty("time").map(cost).toProperty("cost").map(openid).toProperty("openid")
						.map(unionid).toProperty("unionid").map(query).toProperty("query").map(sectionKey)
						.toProperty("sectionKey").map(isDetail).toProperty("isDetail").map(hasResult)
						.toProperty("hasResult").map(result).toProperty("result"));
	}
	...
	default int insertSelective(ProgrametoQueryLog record) {
		return MyBatis3Utils.insert(this::insert, record, programetoQueryLog,
				c -> c.map(time).toPropertyWhenPresent("time", record::getTime).map(cost)
						...
						.toPropertyWhenPresent("unionid", record::getUnionid).map(query)
						.toPropertyWhenPresent("query", record::getQuery).map(sectionKey)
						.toPropertyWhenPresent("sectionKey", record::getSectionKey).map(isDetail)
						.toPropertyWhenPresent("isDetail", record::getIsDetail).map(hasResult)
						.toPropertyWhenPresent("hasResult", record::getHasResult).map(result)
						.toPropertyWhenPresent("result", record::getResult));
	}
	...
	static UpdateDSL<UpdateModel> updateAllColumns(ProgrametoQueryLog record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(time).equalTo(record::getTime).set(cost).equalTo(record::getCost).set(openid)
				.equalTo(record::getOpenid).set(unionid).equalTo(record::getUnionid).set(query)
				.equalTo(record::getQuery).set(sectionKey).equalTo(record::getSectionKey).set(isDetail)
				.equalTo(record::getIsDetail).set(hasResult).equalTo(record::getHasResult).set(result)
				.equalTo(record::getResult);
	}

	@Generated(value = "org.mybatis.generator.api.MyBatisGenerator", comments = "Source Table: programeto_query_log")
	static UpdateDSL<UpdateModel> updateSelectiveColumns(ProgrametoQueryLog record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(time).equalToWhenPresent(record::getTime).set(cost).equalToWhenPresent(record::getCost)
				.set(openid).equalToWhenPresent(record::getOpenid).set(unionid).equalToWhenPresent(record::getUnionid)
				.set(query).equalToWhenPresent(record::getQuery).set(sectionKey)
				.equalToWhenPresent(record::getSectionKey).set(isDetail).equalToWhenPresent(record::getIsDetail)
				.set(hasResult).equalToWhenPresent(record::getHasResult).set(result)
				.equalToWhenPresent(record::getResult);
	}

	@Generated(value = "org.mybatis.generator.api.MyBatisGenerator", comments = "Source Table: programeto_query_log")
	default int updateByPrimaryKey(ProgrametoQueryLog record) {
		return update(c -> c.set(time).equalTo(record::getTime).set(cost).equalTo(record::getCost).set(openid)
				.equalTo(record::getOpenid).set(unionid).equalTo(record::getUnionid).set(query)
				.equalTo(record::getQuery).set(sectionKey).equalTo(record::getSectionKey).set(isDetail)
				.equalTo(record::getIsDetail).set(hasResult).equalTo(record::getHasResult).set(result)
				.equalTo(record::getResult).where(id, isEqualTo(record::getId)));
	}
	...
	default int updateByPrimaryKeySelective(ProgrametoQueryLog record) {
		return update(c -> c.set(time).equalToWhenPresent(record::getTime).set(cost).equalToWhenPresent(record::getCost)
				...
				.equalToWhenPresent(record::getSectionKey).set(isDetail).equalToWhenPresent(record::getIsDetail)
				.set(hasResult).equalToWhenPresent(record::getHasResult).set(result)
				.equalToWhenPresent(record::getResult).where(id, isEqualTo(record::getId)));
	}
}