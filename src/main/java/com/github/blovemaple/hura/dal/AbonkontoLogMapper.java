...
public interface AbonkontoLogMapper {
	...
	BasicColumn[] selectList = BasicColumn.columnList(id, time, status, cost, openid, unionid, msgType, request,
			response);
	...
	default int insert(AbonkontoLog record) {
		return MyBatis3Utils.insert(this::insert, record, abonkontoLog,
				c -> c.map(time).toProperty("time").map(status).toProperty("status").map(cost).toProperty("cost")
						.map(openid).toProperty("openid").map(unionid).toProperty("unionid").map(msgType)
						.toProperty("msgType").map(request).toProperty("request").map(response).toProperty("response"));
	}

	@Generated(value = "org.mybatis.generator.api.MyBatisGenerator", comments = "Source Table: abonkonto_log")
	default int insertSelective(AbonkontoLog record) {
		return MyBatis3Utils.insert(this::insert, record, abonkontoLog,
				c -> c.map(time).toPropertyWhenPresent("time", record::getTime).map(status)
						.toPropertyWhenPresent("status", record::getStatus).map(cost)
						.toPropertyWhenPresent("cost", record::getCost).map(openid)
						.toPropertyWhenPresent("openid", record::getOpenid).map(unionid)
						.toPropertyWhenPresent("unionid", record::getUnionid).map(msgType)
						.toPropertyWhenPresent("msgType", record::getMsgType).map(request)
						.toPropertyWhenPresent("request", record::getRequest).map(response)
						.toPropertyWhenPresent("response", record::getResponse));
	}
	...
	static UpdateDSL<UpdateModel> updateAllColumns(AbonkontoLog record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(time).equalTo(record::getTime).set(status).equalTo(record::getStatus).set(cost)
				.equalTo(record::getCost).set(openid).equalTo(record::getOpenid).set(unionid)
				.equalTo(record::getUnionid).set(msgType).equalTo(record::getMsgType).set(request)
				.equalTo(record::getRequest).set(response).equalTo(record::getResponse);
	}

	@Generated(value = "org.mybatis.generator.api.MyBatisGenerator", comments = "Source Table: abonkonto_log")
	static UpdateDSL<UpdateModel> updateSelectiveColumns(AbonkontoLog record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(time).equalToWhenPresent(record::getTime).set(status).equalToWhenPresent(record::getStatus)
				.set(cost).equalToWhenPresent(record::getCost).set(openid).equalToWhenPresent(record::getOpenid)
				.set(unionid).equalToWhenPresent(record::getUnionid).set(msgType).equalToWhenPresent(record::getMsgType)
				.set(request).equalToWhenPresent(record::getRequest).set(response)
				.equalToWhenPresent(record::getResponse);
	}

	@Generated(value = "org.mybatis.generator.api.MyBatisGenerator", comments = "Source Table: abonkonto_log")
	default int updateByPrimaryKey(AbonkontoLog record) {
		return update(c -> c.set(time).equalTo(record::getTime).set(status).equalTo(record::getStatus).set(cost)
				.equalTo(record::getCost).set(openid).equalTo(record::getOpenid).set(unionid)
				.equalTo(record::getUnionid).set(msgType).equalTo(record::getMsgType).set(request)
				.equalTo(record::getRequest).set(response).equalTo(record::getResponse)
	...
	default int updateByPrimaryKeySelective(AbonkontoLog record) {
		return update(c -> c.set(time).equalToWhenPresent(record::getTime).set(status)
				.equalToWhenPresent(record::getStatus).set(cost).equalToWhenPresent(record::getCost).set(openid)
				.equalToWhenPresent(record::getOpenid).set(unionid).equalToWhenPresent(record::getUnionid).set(msgType)
				.equalToWhenPresent(record::getMsgType).set(request).equalToWhenPresent(record::getRequest)
				.set(response).equalToWhenPresent(record::getResponse).where(id, isEqualTo(record::getId)));
	}
}